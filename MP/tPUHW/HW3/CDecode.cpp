...
void CT1DecodeDirectFetch::show_instruction()
{
    ...
    {
        cout << "MOV3 "
             << "R" << m_instruction.OP1 << ", #" << m_instruction.OP2 << endl;
    }
    else if (m_instruction.OPCODE == ADD)
    {
        unsigned int op2 = (m_instruction.OP2 >> 4) & 0xF;
        cout << "ADD "
             << "R" << m_instruction.OP1 << ", R" << op2 << endl;
    }
      ...
      else if (m_instruction.OPCODE == MOV5)
    {
     ...
     else if(m_instruction.OPCODE == JZ1 )
    {
        cout << "JZ1 " << "R" << m_instruction.OP1 << ", " << m_instruction.OP2 << endl;
    }
      else if(m_instruction.OPCODE == JZ2 )
    {
        cout << "JZ2 " << "R" << m_instruction.OP1 << ", " << m_instruction.OP2 << endl;
    }
     else if(m_instruction.OPCODE == JZ3 )
    {
        cout << "JZ3 " << "R" << m_instruction.OP1 << ", " << m_instruction.OP2 << endl;
    }
    else if (m_instruction.OPCODE == MOV2_)
    {
        unsigned int op2 = m_instruction.OP2 & 0xF;
        cout << "MOV2_ [R" << m_instruction.OP1 << "], R" << op2  << endl;
    }
