...
int CT1ExecuteTinyUnit::do_execute()
{
  ...
  else if (m_decode_unit.get_opcode() == MOV5)
  {
    ...
    //read from memory
    int Mem_Rm = m_mems.read_from_memory(Rm);

    //write on register
    m_regs.write_on_reg(reg_n, Mem_Rm);

    m_regs.set_PC(m_regs.get_PC()+1);
    return clks[MOV5];

  } //ex. JZ1 R0, #3 (R0 <6, then PC+1+3)
  else if(m_decode_unit.get_opcode() == JZ1)
  {
    unsigned int reg_n = m_decode_unit.get_op1();
    int offset = m_decode_unit.get_op2();
  ...
  } //ex. JZ2 R0, #3 (R0 <31, then PC+1+3)
  else if(m_decode_unit.get_opcode() == JZ2)
  {
    unsigned int reg_n = m_decode_unit.get_op1();
    int offset = m_decode_unit.get_op2();
  ...
  } //ex. JZ3 R0, #3 (R0 <76, then PC+1+3)
  else if(m_decode_unit.get_opcode() == JZ3)
  {
    unsigned int reg_n = m_decode_unit.get_op1();
    int offset = m_decode_unit.get_op2();
  ...
  else if (m_decode_unit.get_opcode() == MOV2_)
  {
    ...
    //write on memroy
    m_mems.write_on_memory(Rn-1, Rm);
